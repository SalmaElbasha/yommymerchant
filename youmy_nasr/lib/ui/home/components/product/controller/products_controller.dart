...
class ProductsController extends GetxController {
  ...
  List <ItemDetailsModel> productDetails=[];
  ...
  List<CommentsList>commentsNames=[];
  ...
  Future<void> detailsOfProduct(String itemCode,int branchCode) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("sucessssssss");
        // Successful response
        ItemDetailsModel productDetailModel =
        ItemDetailsModel.fromJson(response.data);
        print(response.data);
       productDetails=[productDetailModel];
        productDetailsD=productDetails.map((productData) {
          return Product(

            rating: productData.itemRate?.toDouble()??0.0,
              branchCode: productData.branchCode,
              id: productData.itemCode??"",
